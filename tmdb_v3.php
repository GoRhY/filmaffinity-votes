...
###############################################################################################################
	...
	*/
		public function searchMovie($movieTitle,$language,$year){
			$movieTitle="query=".urlencode($movieTitle);
			return $this->_call("search/movie",$movieTitle,$language,$year);
		}//end of searchMovie
	...
	*/
	 ...
	 * @return string
	 */
		private function _call($action,$text,$lang="",$year=""){
		// # http://api.themoviedb.org/3/movie/11?api_key=XXX
			$lang=(empty($lang))?$this->getLang():$lang;
			$url= TMDBv3::_API_URL_.$action."?api_key=".$this->getApikey()."&year=".$year."&language=".$lang."&".$text;
			// echo "<pre>$url</pre>";
			$ch = curl_init();
